...
class PlayState extends MusicBeatState
{
	...
	override public function create()
	{
		...
		switch (curStage)
		{
			...
			case 'doors': new states.stages.Doors();
		}
		...
		healthBar = new Bar(boyfriend.x, FlxG.height * (!ClientPrefs.data.downScroll ? 0.89 : 0.11), 'healthBar', function() return health, 0, 2);
		healthBar.leftToRight = false;
		healthBar.scrollFactor.set();
		healthBar.visible = !ClientPrefs.data.hideHud;
		healthBar.alpha = ClientPrefs.data.healthBarAlpha;
		healthBar.scale.x = 0.5;
		...
		var borde1:FlxSprite = new FlxSprite(0,0).makeGraphic(90, FlxG.height, FlxColor.BLACK);
		add(borde1);

		var borde2:FlxSprite = new FlxSprite(FlxG.width - 90, 0).makeGraphic(90, FlxG.height, FlxColor.BLACK);
		add(borde2);

		bar1 = new FlxSprite(0,-150).makeGraphic(FlxG.width + 5, 150, FlxColor.BLACK);
		uiGroup.add(bar1);

		bar2 = new FlxSprite(0,800).makeGraphic(FlxG.width + 5, 150, FlxColor.BLACK);
		uiGroup.add(bar2);

		borde1.cameras = [camOther];
		borde2.cameras = [camOther];
		gameGroup.cameras = [camGame];
	...
	public function triggerEvent(eventName:String, value1:String, value2:String, strumTime:Float) {
		...
		switch(eventName) {
			...
			case 'setZoom':
				...
				var cameraTween2:FlxTween;
				cameraTween2 = FlxTween.tween(FlxG.camera, {zoom: defaultCamZoom}, 0.3, {ease: FlxEase.cubeInOut});

			case 'CameraFlash':
				if(!FlxG.save.data.flashing) {
					if(flValue1 == null) flValue1 = 0.5;
					FlxG.camera.flash(FlxColor.WHITE, flValue1);
				}

			case 'cameraFlashBLACJ':
				if(!FlxG.save.data.flashing) {
					if(flValue1 == null) flValue1 = 0.5;
					FlxG.camera.flash(FlxColor.BLACK, flValue1);
				}

			case 'Cinematic Bars':
				if (flValue1 == 1)
					{
						FlxTween.tween(bar1, {y: 0}, 0.4, {ease: FlxEase.circInOut});
						FlxTween.tween(bar2, {y: 600}, 0.4, {ease: FlxEase.circInOut});

						if (ClientPrefs.data.downScroll)
							{
								for (i in 0...playerStrums.length) {
									FlxTween.tween(playerStrums.members[i], {y: playerStrums.members[i].y - 100}, 0.3, {ease: FlxEase.circInOut});
		
								}
								for (i in 0...opponentStrums.length) {
									FlxTween.tween(opponentStrums.members[i], {y: opponentStrums.members[i].y - 100}, 0.3, {ease: FlxEase.circInOut});
								}
							}
						else
							{
				...
				if (flValue1 == 2)
					{
						FlxTween.tween(bar1, {y: -150}, 0.4, {ease: FlxEase.circInOut});
						FlxTween.tween(bar2, {y: 800}, 0.4, {ease: FlxEase.circInOut});
